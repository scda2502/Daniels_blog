...
def register():
    ...
    if form.validate_on_submit():
        ...
        if User.query.filter_by(email=form.email.data).first() is None:
            print(f"Add user {form.email.data}")
        ...
        flash("You've already signed up with that email. Please try to Log-In")
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        if not current_user.is_authenticated:
            flash("You have to Login to write a comment")
            return redirect(url_for('login'))

        new_comment = Comment(rating = form.rating.data,
    ...
    print(requested_post.blog_comments)
